...
if kubectl get configmap inverse-proxy-config-${JUPYTERLAB_ID}; then
  # If ConfigMap already exist, reuse the existing endpoint (a.k.a BACKEND_ID) and same ProxyUrl.
  PROXY_URL=$(kubectl get configmap inverse-proxy-config-${JUPYTERLAB_ID} -o json | jq -r ".data.ProxyUrl")
  BACKEND_ID=$(kubectl get configmap inverse-proxy-config-${JUPYTERLAB_ID} -o json | jq -r ".data.BackendId")
  run-proxy-agent
  exit 0
fi
...
RESULT_JSON=$(curl -H "Authorization: Bearer $(gcloud auth print-access-token)" -H "X-Inverting-Proxy-VM-ID: ${VM_ID}" -d "${JUPYTERLAB_USER}" "${PROXY_URL}/request-service-account-endpoint" 2>/dev/null)
...
kubectl create configmap inverse-proxy-config-${JUPYTERLAB_ID} \
